...
var T = _interopRequireWildcard(_estransform);

var _scope = require('./scope');

var S = _interopRequireWildcard(_scope);
 ...
 * Import nodes.
 */
/* eslint-disable no-continue, no-cond-assign, no-bitwise, import/prefer-default-export */
const Node = T.Node;
...
function check(condition, message, warn) {
  if (!condition) {
    if (warn) {
      ...
      const loc = logger.context[logger.context.length - 1].loc;
      if (loc) {
...
T.Type.prototype.reflect = function reflect(o) {
  const ty = this.construct().resolve(o.types);
  if (ty !== undefined) {
    ty.lint();
  }
  return ty;
...
T.TypeIdentifier.prototype.construct = function construct() {
  const ty = new TypeAlias(this.name);
  ty.node = this;
  return ty;
};

T.PointerType.prototype.construct = function construct() {
  const ty = new PointerType(this.base.construct());
  ty.node = this;
...
T.ArrayType.prototype.construct = function construct() {
  const ty = new ArrayType(this.base.construct());
  ty.node = this;
  if (this.length) {
...
T.MemberDeclarator.prototype.construct = function construct() {
  return {
...
T.StructType.prototype.construct = function construct() {
  const ty = new StructType(this.id ? this.id.name : undefined);
  const sty = ty.staticType;
  ty.node = this;
  this.members.forEach(m => {
    const member = m.construct();
...
T.ArrowType.prototype.construct = function construct() {
  return new ArrowType(this.params.map(p => p.construct()), this.return.construct());
};
...
PrimitiveType.prototype.resolve = function resolve() {
  return this;
};

TypeAlias.prototype.resolve = function resolve(types, inPointer) {
  startResolving(this);
  check(this.name in types, `unable to resolve type name ${quote(this.name)}`);
  let ty = types[this.name];
...
PointerType.prototype.resolve = function resolve(types) {
  if (this._resolved) {
    return this;
  }
  startResolving(this);
...
ArrayType.prototype.resolve = function resolve(types) {
  if (this._resolved) {
    return this;
  }
  startResolving(this);
...
StructType.prototype.resolve = function resolve(types) {
  if (this._resolved) {
    return this;
  }
  startResolving(this);
...
ArrowType.prototype.resolve = function resolve(types) {
  if (this._resolved) {
    return this;
  }

  const paramTypes = this.paramTypes;
  for (let i = 0, j = paramTypes.length; i < j; i++) {
    if (paramTypes[i]) {
...
PointerType.prototype.lint = function lint() {
...
ArrayType.prototype.lint = function lint() {
  check(this.base, 'array without element type');
  this.base.lint();
  this.align = this.base.align;
};

StructType.prototype.lint = function lint() {
  let maxAlignSize = 1;
  let maxAlignSizeType = Types.u8ty;
  const members = this.members;
  let field;
  let type;
  let prev = { offset: 0, type: { size: 0 } };
  for (let i = 0, j = members.length; i < j; i++) {
...
ArrowType.prototype.lint = function lint() {
  const paramTypes = this.paramTypes;
  for (let i = 0, j = paramTypes.length; i < j; i++) {
    if (paramTypes[i]) {
      paramTypes[i].lint();
...
function resolveAndLintTypes(root, types) {
  const stmts = root.body;
  const aliases = [];
  let s;
  let alias;
  let ty;
  for (let i = 0, j = stmts.length; i < j; i++) {
    s = stmts[i];
  ...
  for (let i = 0, j = aliases.length; i < j; i++) {
    ty = types[aliases[i]];
    logger.push(ty.node);
    ty = ty.resolve(types);
    ty.lint();
...
T.Type.prototype.scan = function scan() {
  return this;
};

Program.prototype.scan = function scan(o) {
  o = extend(o);

  const scope = new Frame(null, 'Program');
...
MemberDeclarator.prototype.scan = function scan(o) {
  if (this.declarator instanceof FunctionDeclaration) {
    this.declarator.scan(o);
  }
};

TypeAliasDirective.prototype.scan = function scan(o) {
  if (this.original instanceof T.StructType) {
    const structName = this.original.id.name;
    const structType = o.types[structName];
    ...
    const members = this.original.members;
    for (let i = 0; i < members.length; i++) {
      const isStatic = members[i].modifiers.indexOf('static');
...
FunctionExpression.prototype.scan = FunctionDeclaration.prototype.scan = function scan(o) {
  logger.push(this);
  let scope = o.scope;
...
VariableDeclaration.prototype.scan = function scan(o) {
  logger.push(this);

  check(this.kind === 'let' || this.kind === 'const' || this.kind === 'extern', `Only block scoped variable declarations are allowed, use the ${quote('let')} keyword instead.`);
...
VariableDeclarator.prototype.scanNode = function scanNode(o) {
  const scope = o.scope;

  const name = this.id.name;
  const ty = this.decltype ? this.decltype.reflect(o) : undefined;

  check(!scope.getVariable(name, true), `Variable ${quote(name)} is already declared in local scope.`);
  scope.addVariable(new Variable(name, ty), o.declkind === 'extern');
};

ForStatement.prototype.scan = function scan(o) {
  o = extend(o);
...
CatchClause.prototype.scan = function scan(o) {
  logger.push(this);
...
BlockStatement.prototype.scan = function scan(o) {
  o = extend(o);
...
PrimitiveType.prototype.assignableFrom = function assignableFrom(other) {
...
StructType.prototype.assignableFrom = function assignableFrom(other) {
  return this === other;
};

PointerType.prototype.assignableFrom = function assignableFrom(other) {
  if (other === Types.nullTy || other instanceof PointerType && this.base.assignableFrom(other.base) || other instanceof PrimitiveType && other.integral) {
    return true;
  }
  return false;
};

ArrowType.prototype.assignableFrom = function assignableFrom(other) {
  if (!(other instanceof ArrowType)) {
    return false;
  }

  const paramTypes = this.paramTypes;
  const otherParamTypes = other.paramTypes;
  ...
  for (let i = 0, j = paramTypes.length; i < j; i++) {
    ...
    } else if (!paramTypes[i].assignableFrom(otherParamTypes[i])) {
      return false;
    }
  }

  for (let i = paramTypes.length, j = otherParamTypes.length; i < j; i++) {
    if (otherParamTypes[i] !== undefined) {
  ...
  for (let i = 0, j = list.length; i < j; i++) {
    ...
    if (trans !== null) {
      if (Array.isArray(trans)) {
        translist.push(...trans.filter(trans => !!trans));
      } else {
        translist.push(trans || list[i]);
      }
    }
  }
  return translist;
...
TypeAliasDirective.prototype.transform = function transform(o) {
  const functions = [];
  if (this.original instanceof T.StructType) {
    const members = this.original.members;
    for (let i = 0; i < members.length; i++) {
      let name = this.original.id.name;
      const member = members[i];
      if (member.declarator instanceof FunctionDeclaration) {
        if (members[i].modifiers.indexOf('static') >= 0) {
          name += '_Static';
        }
        const functionDeclaration = member.declarator;
        const fnName = functionDeclaration.id.name;
...
Program.prototype.transform = function transform(o) {
  o = extend(o);
  o.scope = this.frame;
...
FunctionExpression.prototype.transform = FunctionDeclaration.prototype.transform = function transform(o) {
  o = extend(o);
  o.scope = this.frame;
...
ForStatement.prototype.transform = function transform(o) {
  o = extend(o);
  o.scope = this.scope;
  return Node.prototype.transform.call(this, o);
};

BlockStatement.prototype.transform = function transform(o) {
  o = extend(o);
  o.scope = this.scope;
  this.body = compileList(this.body, o);
  return this;
...
CatchClause.prototype.transform = function transform(o) {
  o = extend(o);
  this.body.transform(o);
  return this;
};

CastExpression.prototype.transform = function transform(o) {
...
Literal.prototype.transformNode = function transformNode() {
  if (this.value === null) {
...
ThisExpression.prototype.transformNode = function transformNode(o) {
  const scope = o.scope;
  const variable = scope.getVariable('this');
  if (variable) {
...
Identifier.prototype.transformNode = function transformNode(o) {
  if (this.kind === 'variable' && !this.variable) {
    ...
    if (!(variable.type instanceof StructStaticType)) {
      check(variable.isStackAllocated ? variable.frame === scope.frame : true, 'cannot close over stack-allocated variables');
    }
...
VariableDeclaration.prototype.transformNode = function transformNode() {
  if (this.kind === 'extern') {
    return null;
  }
  return undefined;
...
VariableDeclarator.prototype.transformNode = function transformNode(o) {
  const variable = this.id.variable;
  ...
  if (this.arguments) {
    ...
    const member = type.getMember(type.name);
    if (member) {
      let constructor = new MemberExpression(new Identifier(`${type.name}$${type.name}`), new Identifier('call'), false);
      constructor = cast(constructor, member.type, true);
      const obj = new UnaryExpression('&', this.id, this.loc).transform(o);
      const callConstructor = new CallExpression(constructor, [obj].concat(this.arguments), this.loc).transform(o);
    ...
    if (this.init instanceof ArrayExpression) {
      const rootPointer = cast(this.id, new PointerType(type.getRoot()), true);
      this.id = o.scope.freshTemp(type, this.loc);
      const elementInitializers = new SequenceExpression([]);
      let generated = 0;
      ...
      function generateInitializers(type, elements) {
        ...
        for (let i = 0; i < elements.length; i++) {
          ...
          if (element instanceof ArrayExpression) {
            ...
            elementInitializers.expressions.push(new AssignmentExpression(new UnaryExpression('*', new BinaryExpression('+', rootPointer, literal(generated++))), '=', element, element.loc));
          }
        }
      }
      ...
      const a = new AssignmentExpression(this.id, '=', this.init, this.init.loc).transform(o);
      this.id = a.left;
      this.init = a.right;
    }
...
ReturnStatement.prototype.transformNode = function transformNode(o) {
  const frame = o.scope.frame;
  const returnType = frame.returnType;
  const arg = this.argument;
  const ty = arg ? arg.ty : undefined;
  if (returnType) {
    check(returnType.assignableFrom(ty), `incompatible types: returning ${quote(Types.tystr(ty, 0))} as ${quote(Types.tystr(returnType, 0))}`);
    if (arg) {
...
ConditionalExpression.prototype.transformNode = function transformNode() {
  let ty;
  const lty = this.consequent.ty;
  const rty = this.alternate.ty;
...
BinaryExpression.prototype.transformNode = function transformNode(o) {
  let ty;
  const lty = this.left.ty;
  const rty = this.right.ty;
  ...
  if (lty instanceof PointerType && (op === '+' || op === '-')) {
    if (rty instanceof PrimitiveType && rty.integral) {
      ...
      check(lty.base.size === rty.base.size, `subtraction with incompatible pointer types ${quote(lty)} and ${quote(rty)}`);
      ty = Types.i32ty;
    }
    ...
    ty = Types.i32ty;
  } else if (BINOP_ARITHMETIC.indexOf(op) >= 0 && lty instanceof PrimitiveType && lty.numeric && rty instanceof PrimitiveType && rty.numeric) {
    // Arithmetic on ints now begets ints, unless it wants to be a wider
    // primitive from an outside cast.
    let wantsToBe;
    if (lty.integral && rty.integral && !((wantsToBe = o.wantsToBe) instanceof PrimitiveType && wantsToBe.size > Types.i32ty.size)) {
...
UnaryExpression.prototype.transform = function transform(o) {
  let ty;
  const op = this.operator;
  ...
  const arg = this.argument = this.argument.transform(o);
  ty = arg.ty;
...
NewExpression.prototype.transform = function transform(o) {
  let ty;
  if (this.callee instanceof Identifier && (ty = o.types[this.callee.name])) {
    const pty = new PointerType(ty);
    let allocation = new CallExpression(o.scope.MALLOC(), [cast(literal(ty.size), Types.u32ty)], this.loc);
    allocation = cast(allocation.transform(o), pty);
    // Check if we have a constructor ArrowType.
    if (ty instanceof StructType) {
      const member = ty.getMember(ty.name);
      if (member) {
        assert(member.type instanceof ArrowType);
        logger.push(this);
        const tmp = o.scope.freshTemp(pty, this.loc);
        const assignment = new AssignmentExpression(tmp, '=', allocation, this.loc);
        let constructor = new MemberExpression(new Identifier(`${ty.name}$${ty.name}`), new Identifier('call'), false);
        constructor = cast(constructor, member.type, true);
        const callConstructor = new CallExpression(constructor, [assignment].concat(this.arguments), this.loc).transform(o);
    ...
    return allocation;
  } else if (this.callee instanceof MemberExpression && this.callee.computed && (ty = o.types[this.callee.object.name])) {
    const size = new BinaryExpression('*', literal(ty.size), this.callee.property, this.loc);
    const allocation = new CallExpression(o.scope.MALLOC(), [cast(size, Types.u32ty)], this.loc);
    return cast(allocation.transform(o), new PointerType(ty));
  }
  return Node.prototype.transform.call(this, o);
};

SequenceExpression.prototype.transformNode = function transformNode() {
  assert(this.expressions.length);
  const last = this.expressions[this.expressions.length - 1];
  return cast(this, last.ty);
};

UpdateExpression.prototype.transformNode = function transformNode(o) {
  const arg = this.argument;
  const ty = arg.ty;
  if (ty && (ty.integral || ty instanceof PointerType)) {
    const scope = o.scope;
    const op = this.operator === '++' ? '+' : '-';
    const ref = scope.cacheReference(arg);
    const right = new BinaryExpression(op, ref.use, literal(1), this.loc);
    if (this.prefix) {
      return new AssignmentExpression(ref.def, '=', right, this.loc).transform(o);
    }
    const t = scope.freshTemp(ty, arg.loc);
    const assn = new AssignmentExpression(t, '=', ref.def, this.loc);
    const incdec = new AssignmentExpression(ref.use, '=', right, this.loc).transform(o);
    return cast(new SequenceExpression([assn, incdec, t], this.loc), ty);
  }
  return undefined;
};

AssignmentExpression.prototype.transformNode = function transformNode(o) {
  const lty = this.left.ty;
  const rty = this.right.ty;
  ...
  if (op !== '=') {
    const binop = op.substr(0, op.indexOf('='));
    const ref = scope.cacheReference(this.left);
    const right = new BinaryExpression(binop, ref.use, this.right, this.right.loc);
    return new AssignmentExpression(ref.def, '=', right, this.loc).transform(o);
  }

  check(lty.assignableFrom(rty), `incompatible types: assigning ${quote(Types.tystr(rty, 0))} to ${quote(Types.tystr(lty, 0))}`);
  ...
  if (lty instanceof StructType) {
    ...
    const memcpyTy = mc.ty.paramTypes[0];
    const left = cast(new UnaryExpression('&', this.left, this.left.loc), memcpyTy, true);
    const right = cast(new UnaryExpression('&', this.right, this.right.loc), memcpyTy, true);
...
MemberExpression.prototype.transformNode = function transformNode(o) {
  let obj = this.object;
  ...
  if (this.kind === '->') {
    check(oty instanceof PointerType && oty.base instanceof StructType, 'base of struct dereference must be struct or union type.');
    oty = oty.base;
  } else {
  ...
  const member = oty.getMember(prop.name);
...
CallExpression.prototype.transformNode = function transformNode(o) {
  if (this.callee instanceof MemberFunctionCall) {
    const obj = this.callee.object;
    const member = this.callee.member;
    const name = `${obj.ty.base.name}$${member.name}`;
    const fn = cast(new MemberExpression(new Identifier(name), new Identifier('call')), member.type, true);
    return new CallExpression(fn, [this.callee.object].concat(this.arguments)).transform(o);
  }

  const fty = this.callee.ty;
  ...
  const paramTys = fty.paramTypes;
  const args = this.arguments;

  check(paramTys.length === args.length, `Argument/parameter count mismatch, expected: ${paramTys.length}, received: ${args.length}`, true);

  for (let i = 0, j = paramTys.length; i < j; i++) {
    const arg = args[i];
    const pty = paramTys[i];
    const aty = arg ? arg.ty : undefined;
    if (pty) {
      if (arg) {
        logger.push(arg);
      }
      check(pty.assignableFrom(aty), `incompatible types: passing ${quote(Types.tystr(aty, 0))} to ${quote(Types.tystr(pty, 0))}`);
      logger.pop();
      args[i] = cast(arg, pty);
    }
  }
...
PrimitiveType.prototype.convert = function convert(expr, force, warn) {
  assert(expr);
  ...
  if (!force) {
    check(!(rty instanceof PointerType), `conversion from pointer to ${quote(Types.tystr(rty, 0))} without cast`, true);
  }
  ...
  let conversion;
  const lwidth = this.size << 3;
  const rwidth = rty ? rty.size << 3 : 8;
  const lsigned = this.signed;
  const rsigned = rty ? rty.signed : undefined;
  const mask = (1 << lwidth) - 1;
  const shift = 32 - lwidth;
  let errorPrefix;

  // If we're converting a constant, check if it fits.
  if (expr instanceof Literal || expr instanceof UnaryExpression && expr.argument instanceof Literal) {
    let val;
    let val2;
  ...
  if (lwidth !== 32 && lwidth < rwidth) {
    ...
    conversion = new BinaryExpression(lsigned ? '|' : '>>>', expr, literal(0), loc);
  }

  return conversion;
};

PointerType.prototype.convert = function convert(expr, force, warn) {
  ...
  if (this === rty || !(rty instanceof PointerType)) {
    if (!force) {
      check(!(rty instanceof PrimitiveType && rty.integral), `conversion from ${quote(Types.tystr(rty, 0))} to pointer without cast`, true);
    }
    return expr;
  }

  if (warn.conversion) {
    check(rty.base.align.size >= this.base.align.size, `incompatible pointer conversion from ${rty.base.align.size}-byte aligned ${quote(Types.tystr(rty, 0))} to ${this.base.align.size}-byte aligned ${quote(Types.tystr(this, 0))}`, true);
  }

  return realign(expr, this.base.align.size);
};

StructType.prototype.convert = function convert(expr) {
  return expr;
};

ArrowType.prototype.convert = function convert(expr) {
  return expr;
};
...
function createPrologue(node) {
  ...
  const frameSize = frame.frameSizeInWords;
  if (frameSize) {
    const allocStack = new AssignmentExpression(frame.realSP(), '-=', literal(frameSize));
    const spDecl = new VariableDeclarator(frame.SP(), allocStack);
  ...
  if (node.parameters) {
    ...
    for (let i = 0, j = params.length; i < j; i++) {
      ...
      const assn = new AssignmentExpression(p.getStackAccess(frame), '=', new Identifier(p.name));
...
Program.prototype.lower = function lower(o) {
  o = extend(o);
  o.scope = o.frame = this.frame;

  this.body = lowerList(this.body, o);
  const prologue = createPrologue(this, o);
  const epilogue = createEpilogue(this, o);
  this.body = prologue.concat(this.body).concat(epilogue);

  return this;
};

FunctionExpression.prototype.lower = FunctionDeclaration.prototype.lower = function lower(o) {
  let memcheckName;
  o = extend(o);
  o.scope = o.frame = this.frame;
  ...
  if (o.memcheck) {
    ...
    this.frame.memcheckFnLoc = {
      name: memcheckName,
      line: this.loc.start.line,
      column: this.loc.start.column
    };
    this.body.body.unshift(new ExpressionStatement(new CallExpression(this.frame.MEMCHECK_CALL_PUSH(), [literal(memcheckName), literal(o.name), literal(this.loc.start.line), literal(this.loc.start.column)])));
...
ForStatement.prototype.lower = function lower(o) {
  o = extend(o);
  o.scope = this.scope;
  return Node.prototype.lower.call(this, o);
};

BlockStatement.prototype.lower = function lower(o) {
  o = extend(o);
  o.scope = this.scope;
  this.body = lowerList(this.body, o);
  return this;
...
CatchClause.prototype.lower = function lower(o) {
  o = extend(o);
  if (o.memcheck) {
    const fnLoc = o.scope.frame.memcheckFnLoc;
    this.body.body.unshift(new ExpressionStatement(new CallExpression(o.scope.frame.MEMCHECK_CALL_RESET(), [literal(fnLoc.name), literal(fnLoc.line), literal(fnLoc.column)])));
  }
  return Node.prototype.lower.call(this, o);
};

Identifier.prototype.lowerNode = function lowerNode(o) {
...
VariableDeclaration.prototype.lowerNode = function lowerNode() {
...
VariableDeclarator.prototype.lowerNode = function lowerNode(o) {
...
ReturnStatement.prototype.lowerNode = function lowerNode(o) {
  const scope = o.scope;
  const frameSize = scope.frame.frameSizeInWords;
  if (frameSize || o.memcheck) {
    const arg = this.argument;
    const t = scope.freshTemp(arg.ty, arg.loc);
    const assn = new AssignmentExpression(t, '=', arg, arg.loc);
    const exprList = [assn];
    if (frameSize) {
      const restoreStack = new AssignmentExpression(scope.frame.realSP(), '+=', literal(frameSize));
      exprList.push(restoreStack);
    }
    if (o.memcheck) {
      const popMemcheck = new CallExpression(scope.MEMCHECK_CALL_POP(), []);
...
BinaryExpression.prototype.lowerNode = function lowerNode() {
  const ty = this.ty;
  const op = this.operator;
  ...
  if (ty instanceof PointerType && (op === '+' || op === '-')) {
    ...
    const rty = this.right.ty;
    if (rty instanceof PrimitiveType && rty.integral) {
      const scale = lty.base.size / lty.base.align.size;
      if (scale > 1) {
...
UnaryExpression.prototype.lowerNode = function lowerNode(o) {
  const arg = this.argument;
...
MemberExpression.prototype.lowerNode = function lowerNode(o) {
...
CastExpression.prototype.lowerNode = function lowerNode(o) {
  // Treat user casts as forced casts so we don't emit warnings.
  const lowered = this.ty.convert(this.argument, !!this.as || this.force, o.warn);
...
function createModule(program, name, bare, loadInstead, memcheck) {
  let body = [];
  const cachedMEMORY = program.frame.cachedMEMORY;
  if (cachedMEMORY) {
    let mdecl;
    ...
    if (name === 'memory') {
      ...
      mdecl = new VariableDeclarator(cachedMEMORY, new SequenceExpression([new CallExpression(new Identifier('load'), [literal('memory.js')]), new Identifier('memory')]));
    } else {
    ...
    if (name !== 'memory') {
      ...
      body.push(new ExpressionStatement(new CallExpression(new MemberExpression(cachedMEMORY, new Identifier('set_memcheck')), [literal(memcheck)])));
    }
  }
  ...
  let mname = name.replace(/[^\w]/g, '_');
  if (mname.match(/^[0-9]/)) {
    mname = `_${mname}`;
  }
  const exports = new Identifier('exports');
  const module = new MemberExpression(new FunctionExpression(null, [exports], body), new Identifier('call'));
  const moduleArgs = [new ThisExpression(), new ConditionalExpression(new BinaryExpression('===', new UnaryExpression('typeof', exports), literal('undefined')), new AssignmentExpression(new Identifier(mname), '=', new ObjectExpression([])), exports)];
...
function compile(node, name, _logger, options) {
  ...
  // Pass 1.
  logger.info('Pass 1');
  const types = resolveAndLintTypes(node, clone(Types.builtinTypes));
  const o = {
    types,
    name,
    logger: _logger,
