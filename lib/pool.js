 ...
 */
   ...
   */
      ...
      function trace() {
        ...
        console.log('      ID Map: ' + getSizeName(bit.length * 4) + 'B');
        console.log('   Count Map: ' + getSizeName(ref.length * 2) + 'B');
        console.log('  Object Map: ' + obj.length);
        console.log('Resize Count: ' + resizeCount);
        var count = 0;
        for (var i = 0; i < bit.length; i++) {
          count += bitCount(bit[i]);
        }
        console.log('Object Map Density: ' + (count / (bit.length * 32) * 100).toFixed(2) + ' %');
      }
      return {
        acquire: acquire,
        release: release,
