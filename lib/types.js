...
class PrimitiveType {
  constructor(name, size, defaultValue, signed) {
    this.name = name;
    this.size = size;
    this.signed = signed;
  ...
  lint() {} // eslint-disable-line class-methods-use-this
}
...
class StructType {
  constructor(name) {
    ...
    this.offset = 0;
    this.isUnion = false;
    this.staticType = this instanceof StructStaticType ? this : new StructStaticType(this);
  }
  ...
  toString(lvl) {
    ...
    let s = `struct${this.name ? ` ${this.name}` : ' '} { `;
    s += this.fields.map(f => `${tystr(f.type, lvl + 1)} ${f.name}`).join('; ');
    return `${s} }`;
  }
...
class StructStaticType extends StructType {
  constructor(type) {
    super(`${type.name}_Static`);
...
class ArrayType extends PointerType {
...
class ArrowType {
  ...
  toString() {
    return `${tystr(this.returnType, 0)}(${this.paramTypes.map(pty => tystr(pty, 0)).join(', ')})`;
  }
}
